...
with col_g1:
    if not df_t_valid.empty and df_t_valid['Tiempo_Atencion_Seg'].sum() > 0:
        ...
        # Guardar figura para el documento Word
        buf_fig1 = BytesIO()
...
with col_t1:
    ...
    analisis_2_1 = st.text_area("Redacte su diagnóstico sobre los trámites:", height=200, key="ans_2_1")
...
cajeros = st.number_input("Número de Cajeros Activos en Ventanilla (c):", min_value=1, max_value=20, value=1)
...
with col_t2:
    ...
    analisis_2_2 = st.text_area("Redacte su diagnóstico sobre la demanda:", height=200, key="ans_2_2")
...
with col_b1:
    link_bpmn = st.text_input("🔗 Link del Diagrama BPMN:")
...
with col_f1:
    ...
    notas_flexsim = st.text_area("Observaciones sobre el modelo en FlexSim:", height=150, key="txt_flexsim")
with col_f2:
...
diagnostico_equipo = st.text_area("Párrafo de Diagnóstico Consolidado:", height=150)
recomendaciones_equipo = st.text_area("Propuestas y Recomendaciones de Intervención:", height=150)
st.markdown("---")
